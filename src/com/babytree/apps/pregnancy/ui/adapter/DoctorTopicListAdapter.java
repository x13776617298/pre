...
public class DoctorTopicListAdapter extends AbstractPageableAdapter<Base> {
	private BabytreeBitmapCache bitmapCache;// 缓存对象
	private Context mContext;
	private AbstractDataLoaderHandler<Base> mHandler;
	private ArrayList<Base> values;

	public DoctorTopicListAdapter(PullToRefreshListView listView, Context context, int loadingViewResourceId,
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		LinearLayout title_bar = viewCache.getTitleBar();
		if (position == 0) {
			title_bar.setVisibility(View.VISIBLE);
			TextView mTxtMessage = viewCache.getTxtMessage();
			mTxtMessage.setText(mHandler.getName() + "(" + mHandler.getMaxItems() + "帖)");
		}
		...
		mLayoutItem.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				MobclickAgent.onEvent(mContext, EventContants.com, EventContants.communicate_topicDetail);
				Discuz discuz = (Discuz) values.get(position);
	...
	private void resetViewHolder(ViewCache viewCache) {
		viewCache.title_bar.setVisibility(View.GONE);
	}
