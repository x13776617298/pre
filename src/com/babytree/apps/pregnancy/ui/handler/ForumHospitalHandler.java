...
public class ForumHospitalHandler implements AbstractDataLoaderHandler<Base>,
	...
	public ForumHospitalHandler(Activity activity, String hospitalId, String isElite) {
		mActivity = activity;
		...
		if (status == REQUEST_SUCCESS) {
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new FirstBackgroundTask(ForumHospitalHandler.this).execute();
				}
			});
		} else if (status == REQUEST_FAILED) {
			MobclickAgent.onEvent(mActivity, EventContants.com, EventContants.communicate_topicListFail);
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new BackgroundTask(ForumHospitalHandler.this).execute(mValues.size());
				}
			});
		}
	}
	...
	public void refershTop(long timestamp) {
		new TopBackgroundTask(ForumHospitalHandler.this).execute(timestamp);
	}
		...
		protected AbstractDataResult<Base> doInBackground(Void... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					pageNo = FIRST_START;
					mValues.clear();
					ret = HospitalController.getDiscuzList(mhospitalID, mIsElite, pageNo);
