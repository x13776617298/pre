...
public class WeekTopicListHandler implements AbstractDataLoaderHandler<Base>,
	...
	private int mPregMonth;

	public WeekTopicListHandler(Activity activity, int pregMonth) {
		mActivity = activity;
		...
		if (status == REQUEST_SUCCESS) {
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new FirstBackgroundTask(WeekTopicListHandler.this).execute();
				}
			});
		} else if (status == REQUEST_FAILED) {
			MobclickAgent.onEvent(mActivity, EventContants.com, EventContants.communicate_topicListFail);
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new BackgroundTask(WeekTopicListHandler.this).execute(mValues.size());
				}
			});
		}
	}
	...
	public void refershTop(long timestamp) {
		new TopBackgroundTask(WeekTopicListHandler.this).execute(timestamp);
	}
		...
		protected AbstractDataResult<Base> doInBackground(Void... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					pageNo = FIRST_START;
					DataResult ret = P_BabytreeController.getDiscuzListByPregMonth(pageNo, mPregMonth);
