...
public class CommentReplyListHandler implements AbstractDataLoaderHandler<Base>,
	...
	public CommentReplyListHandler(Activity activity, String loginString) {
		mActivity = activity;
		...
		if (status == REQUEST_SUCCESS) {
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new FirstBackgroundTask(CommentReplyListHandler.this).execute();
				}
			});
		} else if (status == REQUEST_FAILED) {
			// 错误处理
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new BackgroundTask(CommentReplyListHandler.this).execute(mValues.size());
				}
			});
		}
	}
	...
	public void refershTop(long timestamp) {
		new TopBackgroundTask(CommentReplyListHandler.this).execute(timestamp);
	}
		...
		protected AbstractDataResult<Base> doInBackground(Void... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					pageNo = FIRST_START;
					DataResult ret = MessageController.getMessageListForCommentReply(mLoginString, pageNo);
	...
	private class TopBackgroundTask extends AsyncTask<Long, Void, AbstractDataResult<Base>> {
		...
		protected AbstractDataResult<Base> doInBackground(Long... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					DataResult ret = MessageController.getMessageListForCommentReply(mLoginString, FIRST_START);
					result.status = ret.status;
					if (ret.status == P_BabytreeController.SUCCESS_CODE) {
						mMaxItems = ret.totalSize;
						ArrayList<Base> values = (ArrayList<Base>) ret.data;
