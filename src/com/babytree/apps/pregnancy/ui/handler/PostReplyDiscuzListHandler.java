...
public class PostReplyDiscuzListHandler implements AbstractDataLoaderHandler<Base>,
	...
	public PostReplyDiscuzListHandler(Activity activity, String userEncodeId, String loginString, String type) {
		mActivity = activity;
		mUserEncodeId = userEncodeId;
		mType = type;
		...
		if (status == REQUEST_SUCCESS) {
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new FirstBackgroundTask(PostReplyDiscuzListHandler.this).execute();
				}
			});
		} else if (status == REQUEST_FAILED) {
			MobclickAgent.onEvent(mActivity, EventContants.com, EventContants.communicate_topicListFail);
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new BackgroundTask(PostReplyDiscuzListHandler.this).execute(mValues.size());
				}
			});
		}
	}
	...
	public void refershTop(long timestamp) {
		new TopBackgroundTask(PostReplyDiscuzListHandler.this).execute(timestamp);
	}
		...
		protected AbstractDataResult<Base> doInBackground(Void... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					pageNo = FIRST_START;
					DataResult ret = P_BabytreeController.getUserDiscuzList(mUserEncodeId, mLoginString, mType, pageNo);
	...
	private class TopBackgroundTask extends AsyncTask<Long, Void, AbstractDataResult<Base>> {
		...
		protected AbstractDataResult<Base> doInBackground(Long... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					DataResult ret = P_BabytreeController.getUserDiscuzList(mUserEncodeId, mLoginString, mType,
							FIRST_START);
					result.status = ret.status;
					if (ret.status == P_BabytreeController.SUCCESS_CODE) {
						mMaxItems = ret.totalSize;
