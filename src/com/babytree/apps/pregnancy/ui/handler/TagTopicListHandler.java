...
public class TagTopicListHandler implements AbstractDataLoaderHandler<Base>,
	...
	public TagTopicListHandler(Activity activity, String tag, String loginStr) {
		mActivity = activity;
		mTag = tag;
		...
		if (status == REQUEST_SUCCESS) {
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new FirstBackgroundTask(TagTopicListHandler.this).execute();
				}
			});
		} else if (status == REQUEST_FAILED) {
			MobclickAgent.onEvent(mActivity, EventContants.com, EventContants.communicate_topicListFail);
			...
			btnReload.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					new BackgroundTask(TagTopicListHandler.this).execute(mValues.size());
				}
			});
		}
	}
	...
	public void refershTop(long timestamp) {
		new TopBackgroundTask(TagTopicListHandler.this).execute(timestamp);
	}
		...
		protected AbstractDataResult<Base> doInBackground(Void... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					pageNo = FIRST_START;
					DataResult ret = P_BabytreeController.getDiscuzListByTag(loginStr, pageNo, mTag);
	...
	private class TopBackgroundTask extends AsyncTask<Long, Void, AbstractDataResult<Base>> {
		...
		protected AbstractDataResult<Base> doInBackground(Long... params) {
			...
			try {
				if (com.babytree.apps.comm.util.BabytreeUtil.hasNetwork(mActivity)) {
					DataResult ret = P_BabytreeController.getDiscuzListByTag(loginStr, FIRST_START, mTag);
					result.status = ret.status;
					if (ret.status == P_BabytreeController.SUCCESS_CODE) {
						mMaxItems = ret.totalSize;
						ArrayList<Base> values = (ArrayList<Base>) ret.data;
